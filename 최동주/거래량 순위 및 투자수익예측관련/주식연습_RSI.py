...
try:
    ...
    for i in range(len(df_krx)):
        ...
        if rsi <= 30 and capital > close_price:
            ...
            print(f"{date}: 매수 - {shares_to_buy} 주 @ {close_price}원, 남은 자본: {capital}원 [RSI : {rsi}]")

        # 매도 조건: RSI가 70 이상일 때
        elif rsi >= 70 and shares > 0:
            capital += shares * close_price
            print(f"{date}: 매도 - {shares} 주 @ {close_price}원, 총 자본: {capital}원 [RSI : {rsi}]")
            shares = 0
